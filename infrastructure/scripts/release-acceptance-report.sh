...
RELEASE_GATE_ENFORCE="${RELEASE_GATE_ENFORCE:-1}"
...

{
  ...
  echo "- ReleaseGateEnforce: $RELEASE_GATE_ENFORCE"
...
if [[ "$RELEASE_GATE_ENFORCE" == "1" && "$OVERALL_DECISION" != "PASS" ]]; then
  echo "[gate] BLOCKED: overall decision is $OVERALL_DECISION"
  exit 92
fi
if [[ "$RELEASE_GATE_ENFORCE" == "1" ]]; then
  echo "[gate] PASSED"
fi
